...
:::{.callout-note}
The default branch used to be called `master` and was then changed to `main` (next slide).
:::

## `master` vs. `main` ⚙️

The default branch used to be called `master`, this was more-or-less recently changed to `main`, but not all repositories / git installations have changed yet.

Whether git on your computer uses `master` or `main` depends on when it was installed.
...
2. Write the "Question" with the letters A to C into the file, with empty lines in between e.g.
...
a) What percentage of developers report using git for version control in 2025?
b) What fraction of long distance trains in Germany were on time in May 2026? 
c) How many liters of Beer have been drunken at the Oktoberfest in Munich in 2019? 🍻
...
```text
Question A 🧑🏻‍💻 git users [^1]
93%
Question B 🚂 Trains on time [^2]
61.2%
Question C 🍻 Liters of beer [^3]
7,850,200
```
...
[^3]: https://opendata.muenchen.de/tr/dataset/oktoberfest
